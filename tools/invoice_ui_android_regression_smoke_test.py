...
items = (ROOT / "src/nano_offline/views/items_view.py").read_text(encoding="utf-8")
...
# The "materials" (items) screen used to set its header inline in main.py;
# it now lives in ItemsCenter (nano_offline/views/items_view.py) after the
# 2026-08 refactor that split main.py's view closures into Center classes.
# Same functional guarantee, new location.
assert 'self._set_header("المواد"' in items
assert '"التصنيفات والوحدات"' in items
