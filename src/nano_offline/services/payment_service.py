...
class PaymentService:
    ...
    def _display_amount(self, value: float | int | None) -> str:
        """Format a stored USD amount for user-facing messages, in the currently displayed currency."""
        with self.db.connect() as conn:
            settings = {str(r["key"]): str(r["value"]) for r in conn.execute("SELECT key,value FROM settings").fetchall()}
        return currency.format_amount(value, settings)
        ...
        if mode == "manual":
            ...
            for invoice_id, raw_amount in manual.items():
                ...
                if alloc > remaining + EPSILON:
                    raise ValueError(
                        f"المبلغ الموزع على الفاتورة #{invoice_id} يتجاوز المتبقي {self._display_amount(remaining)}"
                    )
                used += alloc
