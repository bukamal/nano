...
class ActivationGate:
    ...
    async def _activate(self, _):
        self._set_busy(True)
        try:
            status = await asyncio.to_thread(self.ctx.license.activate_online, self.key.value or "", "0.7.2")
            if not status.valid:
                ...
                ft.Column(
                    [
                        ft.Container(
                            ft.Column(
                                [
                                    ft.Image(src="icon.png", width=88, height=88, fit=ft.ImageFit.CONTAIN),
                                    ft.Text("تفعيل Nano | نانو", size=26, weight=ft.FontWeight.BOLD, color="#0A2E78"),
                                    ft.Text(
                                        "يستخدم نفس سيرفر تفعيل هوى الشام. الاتصال مطلوب عند التفعيل فقط، وبعد نجاحه تبقى بيانات Nano والمحاسبة محلية على الجهاز.",
                                        size=12,
                                        color="#475569",
                                        text_align=ft.TextAlign.CENTER,
                                    ),
