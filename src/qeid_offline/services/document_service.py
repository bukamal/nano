...
class DocumentService:
    ...
    def _shell(self, title: str, body: str) -> str:
        ...
        company = self._e(settings.get("company_name") or "قيد")
...
.sheet {{ width:100%; }}
.header {{ border-bottom:3px solid #0F4C81; padding-bottom:10px; margin-bottom:14px; }}
.brand {{ color:#0F4C81; font-size:24px; font-weight:700; }}
...
th {{ background:#0F4C81; color:#fff; padding:7px 5px; border:1px solid #0F4C81; }}
...
{body}
<div class="footer">تم إنشاء المستند محليًا بواسطة قيد - أوفلاين</div>
</div></body></html>"""
