...
class BackupService:
    ...
    def create_backup(self, destination: str | Path) -> Path:
        ...
        if destination.suffix.lower() != ".qeidbackup":
            destination = destination.with_suffix(destination.suffix + ".qeidbackup" if destination.suffix else ".qeidbackup")
        destination.parent.mkdir(parents=True, exist_ok=True)
        self.db.checkpoint()

        with tempfile.TemporaryDirectory(prefix="qeid_backup_") as td:
            temp_db = Path(td) / "qeid.db"
            ...
            temp_zip = Path(td) / "backup.qeidbackup"
            with zipfile.ZipFile(temp_zip, "w", compression=zipfile.ZIP_DEFLATED) as zf:
                zf.write(temp_db, arcname="qeid.db")
    ...
    def validate_backup(self, backup_path: str | Path) -> BackupValidation:
        ...
        with tempfile.TemporaryDirectory(prefix="qeid_validate_") as td:
            out = Path(td)
            try:
                with zipfile.ZipFile(backup_path, "r") as zf:
                    names = set(zf.namelist())
                    if {"qeid.db", "manifest.json"} - names:
    ...
    def restore_backup(self, backup_path: str | Path) -> Path:
        ...
        safety_path = safety_dir / f"pre_restore_{datetime.now().strftime('%Y%m%d_%H%M%S')}.qeidbackup"
        self.create_backup(safety_path)

        with tempfile.TemporaryDirectory(prefix="qeid_restore_") as td:
            out = Path(td)
            with zipfile.ZipFile(backup_path, "r") as zf:
                zf.extract("qeid.db", out)
            source_db = out / "qeid.db"
