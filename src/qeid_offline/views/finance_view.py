    ...
    def show_statements(self, party_type: str) -> None:
        ...
        async def print_statement(_):
            ...
            try:
                ...
                await self.native_files.print_html(html, name=f"qeid-statement-{party_type}-{party_dd.value}")
            except Exception as exc:
                self.notify(str(exc))
        ...
        async def share_statement_pdf(_):
            ...
            try:
                ...
                await self.native_files.share_pdf(
                    html, filename=f"qeid_statement_{party_type}_{party_dd.value}.pdf"
                )
            except Exception as exc:
                self.notify(str(exc))
